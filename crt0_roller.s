...
; This one is hacked a bit - it has the spinner code (which is normal),
...
start:
	; clear NMI interrupt flag right off
	ld hl,#_vdpLimi
	ld (hl),#0
	...
	im 1					; spinner interrupts use mode 1 (check BIOS, may already be on)
	ei						; and now they may go 
...
spin_cleanup:
	...
	pop hl					
	pop af
	ei						; by doing the EI this late, no other maskable interrupts can
	reti					; occur. But does the Coleco HAVE any others?
